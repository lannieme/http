...
int close_socket(int sock)
{
    ...
    fprintf(server_log, "Closed socket.\n");
    return 0;
}
...
{   
  ...
  fprintf(server_log, "Server socket successfully created.\n");
  ...
  while (1)
  {
    ...
    if (select(fdmax+1, &read_fds, NULL, NULL, NULL) == -1) {
        perror("select");
        fprintf(server_log, "ERROR: not able to select");
        exit(4);
    }
    ...
    for(i = 0; i <= fdmax; i++) {
        if (FD_ISSET(i, &read_fds)) { // we got one!!
            if (i == sock) {
                ...
                if (client_sock == -1) {
                    perror("accept");
                    fprintf(server_log, "ERROR: not able to accept connection");
                } else {
                    ...
                    printf("selectserver: new connection from %s on "
                        ...
                        client_sock);
                    fprintf(server_log, "selectserver: new connection from %s on "
                        "socket %d\n",
                        inet_ntop(cli_addr.sin_family,
                            get_in_addr((struct sockaddr*)&cli_addr),
                            remoteIP, INET6_ADDRSTRLEN),
                ...
                if ((nbytes = recv(i, buf, sizeof buf, 0)) <= 0) {
                    ...
                    if (nbytes == 0) {
                        ...
                        fprintf(server_log, "selectserver: socket %d hung up\n", i);
                    } else {
                        perror("recv");
                        fprintf(server_log, "ERROR: not able to receive data from socket %d\n",i );
                    }
                    close(i); // bye!
                    ...
                    if (status  == -1) {
                      perror("send");
                      fprintf(server_log, "ERROR：not able to sent data to client");
                    }
                }
